...
## Replacing the standalone prototype

This branch replaces the development-only standalone runtime design. Remove
`agents.defaults.embeddedAgent.runtimeServer` from prototype Gateway configurations
and stop invoking the prototype `runtime-server` CLI. Those paths are removed,
not aliases for worker placement. Preserve a configuration backup, then use the
node/profile setup below and your normal service lifecycle. The implementation
does not edit live configuration or stop existing services for you.
