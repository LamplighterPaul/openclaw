...
it.each(["approval.get", "approval.resolve"] as const)(
  "retains terminal truth for %s after reconciliation retires its live registration",
  async (method) => {
    const { managers, handlers, record, stored, deny, decision } = createPendingFixture();
    const nowMs = Date.now();
    vi.spyOn(operatorApprovalStore, "getOperatorApprovalDetailedAsync").mockResolvedValue({
      outcome: "found",
      record: {
        ...stored,
        status: "denied",
    ...
    const reconcile = managers.exec.reconcileDurableLookup.bind(managers.exec);
    vi.spyOn(managers.exec, "reconcileDurableLookup").mockImplementation((lookup, resolvedBy) => {
      const terminal = reconcile(lookup, resolvedBy);
      managers.exec.retire();
      return terminal;
    });
    try {
      const response = await invoke({
        ...
        body:
          method === "approval.get"
            ? { id: record.id }
            : { id: record.id, kind: "exec", decision: "allow-once" },
        client: createClient({ deviceId: "reviewer" }),
      });
      expect(response).toMatchObject({
        ok: true,
        result: {
          ...(method === "approval.resolve" ? { applied: false } : {}),
          approval: { status: "denied" },
        },
      });
      await expect(decision).resolves.toBe("deny");
      expect(managers.exec.getLiveSnapshot(record.id)).toBeNull();
